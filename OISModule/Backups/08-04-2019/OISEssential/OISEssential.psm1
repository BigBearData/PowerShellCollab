...
$ScriptBlock = {Get-WSManInstance -ResourceURI winrm/config/listener -SelectorSet @{Address="*";Transport="http"} }
	 try { 
		...
		Write-Host "Invoke-Command does not work for $ServerName using the -ComputerName parameter" -ForegroundColor Red 
		}
...
$SSRSPath = OIS_XML_GetSQLConfig -Command SSRSPath 
$SSRSConfigFile = $SSRSPath + "\"+$SSRSConfigFileName
#$SSRSConfigFile

	if (Get-Content $SSRSConfigFile) {
		[xml]$RSReportServerconfig = Get-Content $SSRSConfigFile
		$RSReportServerconfig.Configuration.Authentication.AuthenticationTypes
		#$AuthType.RSWindowsNegotiate
		#$RSReportServerconfig.SelectSingleNode("//AuthenticationTypes")
	} else {
...
Write-Host "setspn -s http/$ServerName $Domain\$UserName"
Write-Host "setspn -s http/$ServerName.$FQDN $Domain\$UserName"

If ($IISBinding) {
	Write-Host "setspn -s http/$IISBinding $Domain\$UserName"
	Write-Host "setspn -s http/$IISBinding.$FQDN $Domain\$UserName"
	}
...
switch($ServiceName.ToUpper()) {
	...
	MSSQL {
		...
		Write-host "Gettig $ServiceName information from Localhost $ServerName"
...
$Result = Get-ADUser -LDAPFilter "(SamAccountname=$EntUserName)" -Properties name, serviceprincipalname -ErrorAction Stop | Select-Object @{Label = "Service Principal Names";Expression = {$_.serviceprincipalname}} | Select-Object -ExpandProperty "Service Principal Names" 
	...
	{ 
		...
		Write-host " " #adds a space before the line below 
		OIS_SuggesteUserSPNs -UserName $EntUserName -ServerName ServerName
		Write-host " " #adds a space before the line below 
	}  
<#
.SYNOPSIS
...
$GetInstalledPrg #>
#$SoftwSMO = $EnterpriseServer | .\Get-InstalledProgram.ps1 -PN "*Shared Management Objects" -Property DisplayName,DisplayVersion | format-table
$SoftwSMO = $EnterpriseServer | .\Get-InstalledProgram.ps1 -PN "*ble" -Property DisplayName,DisplayVersion | format-table
If ($SoftwSMO) {
	$SoftwSMO
}
elseif (!$SoftwSMO) {
...
If ($InvokeCommandCheckMSSQL -eq $True) {
	Write-host " "
	...
	#OIS_GetInstalledSoftware -ServerName $SQLServer -SoftwareName "*Management Studio*"
	#OIS_GetInstalledSoftware -ServerName $SQLServer -SoftwareName "*Native Client*"
	$SQLServer | .\Get-InstalledProgram.ps1 -PN "*Management Studio*" -Property DisplayName,DisplayVersion | format-table
	$SQLServer | .\Get-InstalledProgram.ps1 -PN "*Native Client*" -Property DisplayName,DisplayVersion | format-table 
...
If ($InvokeCommandCheckSSIS -eq $True) {
	...
	#OIS_GetInstalledSoftware -SoftwareName "*data tools*" -ServerName $SSISServer
	#OIS_GetInstalledSoftware -SoftwareName "*Native Client*" -ServerName $SSISServer
	Write-host " "
	Write-host "Checking Installed Software" -ForegroundColor Yellow
	$SSISServer | .\Get-InstalledProgram.ps1 -PN "*data tools*" -Property DisplayName,DisplayVersion | format-table
	$SSISServer | .\Get-InstalledProgram.ps1 -PN "*Native Client*" -Property DisplayName,DisplayVersion | format-table
	Write-host "Checking Network Prerequisites Requirements for Data Warehouse (MSSQL) Server" -ForegroundColor Yellow
	Write-host " "
	OIS_CheckPSModule -ModuleName SqlServer
	OIS_CheckPSModule -ModuleName SQLPS
...
$InvokeCommandCheckSSRS = OIS_TryInvokeCommand -ServerName $SSRSServer
	...
	If ($InvokeCommandCheckSSRS -eq $True) {
		...
		#OIS_GetInstalledSoftware -SoftwareName "*Management Studio*" -ServerName $SSRSServer
		#OIS_GetInstalledSoftware -SoftwareName "*Native Client*" -ServerName $SSRSServer
		#"Server1","Server2" | Get-InstalledProgram -PN "Adobe Acrobat*" 
		#PS C:\Program Files\WindowsPowerShell\Modules\OISEssential> "salesdemo" | .\Get-InstalledProgram.ps1 -PN 'Microsoft*' -Property DisplayName,DisplayVersion,Publisher,InstallLocation
		Write-host "Checking Installed Software" -ForegroundColor Yellow
		$SSRSServer | .\Get-InstalledProgram.ps1 -PN "*Management Studio*" -Property DisplayName,DisplayVersion | format-table
		$SSRSServer | .\Get-InstalledProgram.ps1 -PN "*Native Client*" -Property DisplayName,DisplayVersion | format-table
		Write-host "Checking Authentication Methods: $AuthMethod "
		#Write-host " "
		OIS_SSRS_GetAuthMethod | format-list
		Write-host "Checking Network Prerequisites Requirements for Data Warehouse (MSSQL) Server" -ForegroundColor Yellow
