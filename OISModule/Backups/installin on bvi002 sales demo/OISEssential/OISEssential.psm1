...
[string]$SSRSConfigPath = $SSRSPath -Replace ("bin","RSReportServer.config")
 	...
 	if (Get-Content $SSRSConfigPath) {
		#$SSRSConfigPath
		[XML]$RSReportServerconfig = Get-Content -Path $SSRSConfigPath
...
<# 		if (!$SQLServer) {
		...
		if (!$SQLUser) {
		$SQLUser = OIS_GetServiceUser -ServiceName MSSQL -CheckRemote $CheckRemote -ServerName $SQLServer
		}
...
If ($InvokeCommandCheckMSSQL -eq $True -Or $CheckRemote -eq $False) {
	...
	If ($ODWUser -match "\\") {OIS_GetEntUserSPN -EntUserName $ODWUser.split("\")[1] } ElseIf ($ODWUser -match "@") {OIS_GetEntUserSPN -EntUserName $ODWUser.split("@")[0] } Else {OIS_GetEntUserSPN -EntUserName $ODWUser}
...
<# 		if (!$SSRSServer) {
		...
		if (!$SSRSUser) {
		$SSRSUser = OIS_GetServiceUser -ServiceName SSRS -Verbose $V -CheckRemote $CheckRemote #OIS_GetSSRSUser
		}
...
#$InvokeCommandCheckSSRS
	...
	If ($InvokeCommandCheckSSRS -eq $True -Or $CheckRemote -eq $False) {
		...
		If ($SSRSUser -match "\@") {OIS_GetEntUserSPN -EntUserName $SSRSUser.split("@")[0] } Elseif ($SSRSUser -match "\\") {OIS_GetEntUserSPN -EntUserName $SSRSUser.split("\")[1] } Else {OIS_GetEntUserSPN -EntUserName $SSRSUser}  #-ErrorAction SilentlyContinue 
