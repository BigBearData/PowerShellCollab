...
$ServerName.ToUpper() -notmatch $LocalComputerName
#$LocalComputerName
#$ServerName
...
$ScriptBlock = {Get-WSManInstance -ResourceURI winrm/config/listener -SelectorSet @{Address="*";Transport="http"} }
	...
	Else { 
		Write-Host "$ServerName is not a remote host, Invoke-Command will not be tested." -ForegroundColor Red 
		$InvokeCommandCheck
		}
...
#$SSRSConfigFile
	...
	if (Get-Content $SSRSConfigFile) {
		...
		$RSReportServerconfig.Configuration.Authentication.AuthenticationTypes #BUG: should not show errors if file not found.
		#$AuthType.RSWindowsNegotiate
...
switch($ServiceName.ToUpper()) {
	...
	MSSQL {
		...
		$ServiceInfo.PathName | Select-String '^"?(.+)\.exe' | ForEach-Object {
...
$SSRSresultsarray =@("Summary of SSRS prerequisites testing:")
...
#$InvokeCommandCheckSSRS
	...
	If ($InvokeCommandCheckSSRS -eq $True -Or $CheckRemote -eq $False) {
		...
		#"Server1","Server2" | Get-InstalledProgram -PN "Adobe Acrobat*" 
		#PS C:\Program Files\WindowsPowerShell\Modules\OISEssential> "salesdemo" | OIS_GetInstalledPrograms -PN 'Microsoft*' -Property DisplayName,DisplayVersion,Publisher,InstallLocation
		Write-host "Checking Installed Software" -ForegroundColor Yellow
