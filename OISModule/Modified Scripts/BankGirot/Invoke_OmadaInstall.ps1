...
Function Invoke-OmadaInstall {
    ...
    try{
        ...
        $serviceUserDomain = $env:userdomain #$localConfiguration.Service.Domain
...
#		$User = Get-ADUser -Filter {sAMAccountName -eq $serviceUser}
		If ($null -eq $User){
			...
			$serviceUserPassword = $localConfiguration.Service.Password
			$serviceUserPassword = "HOsdl8_12yd5S"# Read-PasswordFromConfig -XMLPath $XMLPath -encryptedText $serviceUserPassword
    ...
    if ($installES -or $isFullInstall){
	    ...
	    try{
            ...
            if ($SSISInstance -ne 'localhost' -and !$SSISInstance.startswith($env:ComputerName)){
				...
				#-ComputerName $SSISInstance -Credential $credDB 
    ...
    try{
        if ($SSISInstance -eq $SQLInstanceWithout){
			...
			# -ComputerName $SSISInstance -Credential $credDB 
            Invoke-Command -ScriptBlock $ScriptBlock -ArgumentList $SQLVersionNo, $SQLInstance, $dtexecDir
    ...
    if ($installES -eq $true){
        ...
        try{
            ...
            Catch {
                ...
                if ($SSISInstance -ne 'localhost' -and !$SSISInstance.startswith($env:ComputerName)){
                    Invoke-Command -ScriptBlock $ScriptBlock -ArgumentList $esBinding, $ip, $false
                }
    ...
    if ($installODW -eq $true){
        ...
        try{
            ...
            Invoke-Command -ScriptBlock {
#			Invoke-Command -Credential $credDB -ScriptBlock {
				 $SQLVersionNo = $args[0]
				 $dtexecDir = $args[1]
    ...
    try{
        ...
        if (($installChangesets -eq $true) -and ($installES -eq $true) -and ($installODW -eq $true)){
            try{
				...
				#-Credential $credDB -ComputerName $SSISInstance 
                    Invoke-Command -ScriptBlock $ScriptBlock -ArgumentList $sqlFile, $esAuditDBName, $SQLAdmUser, $SQLAdmPass, $SQLInstance, $useSQLUser
                }
    ...
    if ($installODW){
         ...
         try{
            if ($disableForceSSL -eq $true){
				...
				#-Credential $CredDB -ComputerName 
                    Invoke-Command -ScriptBlock $ScriptBlock $SQLInstanceWithout -ArgumentList $SSRSPath, $SQLInstanceWithout
                }
                Show-Info -IsCI $IsCI -Message "***************************" -ForegroundColor Green
            }
            else{
...
# add different users in different databases - if needed
    ...
    if ($demoEnabled){
        ...
        if ($installRoPE -eq $true -and $stopRopeService -eq $true){#first start of rope service
            Restart-Service -ServiceName "RoPE_0"  -Action "Start" #("*{0}*" -f $ropeServiceName)
        }
    }
	...
	if ($remoteDB -and !$SSISInstance.startswith($env:ComputerName)){
		Try{
			Show-Info -IsCI $IsCI -Message "6.13 Removal of network shares created during installation" -ForegroundColor DarkGreen
			$c = {
	...
	if ($SSISInstance -ne 'localhost' -and !$SSISInstance.startswith($env:ComputerName) -and $remoteDB){
		...
		Invoke-Command -ScriptBlock $ScriptBlock -ArgumentList $SSISInstance
