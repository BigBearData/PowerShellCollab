Function Publish-Reports {
    ...
    .PARAMETER skipErrors
        Switch if reports upload errors should be checked (and stop the build if they exist)
    .PARAMETER credDB
    ...
    Show-Info -IsCI $IsCI -Message "Creating batch file to upload reports..." -ForegroundColor Yellow
    $s = ""
    if ($rsHttps -eq $true) {
        $s = "s"
    }
    if ($remoteDB -and $rsOnAppServer) {
        ...
        $ScriptBlock = {
            ...
            $c = Get-Content -Encoding UTF8 ("{0}\NativeLoadReports.bat.template" -F $odwUploadReportsToolPath)
            ...
            Write-Host  "Uplading reports..." -ForegroundColor Yellow

            $f = ('"{0}\NativeLoadReports.bat" > "{1}"' -F $odwUploadReportsToolPath, $logFile)
            Invoke-Expression "& $f"
            
            if (!($skipErrors)){
                    #Verify if there was an error during report upload...
                $errors = @('The request failed with HTTP status', 'The operation has timed out', 'SoapException')
                $uploadFail = $false
                $errors | ForEach-Object {
                    $c = Get-Content -Path $logFile
                    if ($c -match $_){
                        $uploadFail = $true
                    }
                }
                if ($uploadFail){
                    Write-Error "There was an error while uploading reports"
                    Throw
                }
            }else{
                Write-Host "Error checking is disabled" -ForegroundColor Yellow
            }
            

            Write-Host  "Reports added" -ForegroundColor Green
            #remove folder
        }
        ...
        if ($SQLInstanceName -ne $SQLInstanceWithout) {
            $rsUrl = ("http{0}://{1}/ReportServer_{2}/" -F $s, 'localhost', $SQLInstanceName)#$SQLInstanceWithout
        }
        else {
            $rsUrl = ("http{0}://{1}/ReportServer/" -F $s, 'localhost')#$rsServer
        }
        $ScriptBlock = {
            ...
            if ((Test-Path -Path $ssrsPath)) {
                ...
                if (!(Test-Path -Path $toolPath)) {
                    ...
                    if (Test-Path -Path (Join-Path -Path $odwInstallationPath -ChildPath "Support Files\Omada.ODW.SSRS.Utils.dll")) {
                        Write-Host "Omada.ODW.SSRS.Utils.dll is missing - fixing" -ForegroundColor Yellow
                ...
                Write-Host  "Reporting server path not found, not critical error - skipping" -ForegroundColor Red
                Write-Host  ("({0}) {1}" -F $server, $ssrsPath) -ForegroundColor Red
            }
        }
